...
export function registerRoutes(app) {
  ...
  app.get(
    "/api/v2/hashpower/rented-summary",
    asyncHandler(async (req, res) => {
      ...
      const results = await Promise.all(
        nhAccounts.map(async (acct) => {
          const { client, clientName } = resolveNhClient(acct);
          if (
            !client ||
            (acct !== "BT" && clientName === "BT" && acct !== "PH")
          )
            return null;
          try {
      ...
      results.filter(Boolean).forEach(({ clientName, list }) => {
        list.forEach((o) => {
          const status =
            typeof o.status === "object" ? o.status.code : o.status;
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        const wait = 3000;
        const hostInput = await driver.wait(
          until.elementLocated(By.css('input[placeholder*="stratum"]')),
          wait,
        );
        ...
        const resultSection = await driver.wait(
          until.elementLocated(By.className("verification-results")),
          5000,
        );
        ...
        const isSuccess =
          ...
          resultText.toLowerCase().includes("verified");
  ...
  app.post(
    "/api/v2/mrr/monitor/run",
    asyncHandler(async (req, res) => {
      const scope = String(req.query.client || req.body?.client || "ALL")
        .trim()
        .toUpperCase();
  ...
  app.get(
    "/api/v2/mrr/monitor/snapshot",
    asyncHandler(async (req, res) => {
      db.all(
        `SELECT * FROM rentals ORDER BY last_updated DESC`,
        [],
        (err, rows) => {
  ...
  app.delete(
    "/api/v2/mrr/monitor/snapshot/:id",
    asyncHandler(async (req, res) => {
      const { id } = req.params;
  ...
  app.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      db.run(
        `UPDATE rentals SET ${fields} WHERE id = ?`,
        values,
        function (err) {
          if (err) return res.status(500).json({ error: err.message });
          res.json({ success: true, changes: this.changes });
        ...
        const results = await Promise.all(
          allClientNames.map(async (clientName) => {
            try {
              ...
              const rigs = Array.isArray(rigsData?.data)
                ? rigsData.data
                : Array.isArray(rigsData?.data?.rigs)
                  ? rigsData.data.rigs
                  : [];
              if (rigsData?.success && rigs.length > 0) {
                ...
                if (poolsData?.success) {
                  const items = (
                    ...
                    mrrClient: clientName,
                    nicehashAlgo: normalizeAlgoForNiceHash(
                      item.algo || item.algorithm || item.type,
                    ),
                  }));
                  return {
                    pools: items.map((item) => ({
                      ...item,
                      mrrClient: clientName,
                    })),
                  };
  ...
  app.get(
    "/api/v2/mrr/balance",
    asyncHandler(async (req, res) => mrrRequest("/account/balance", req, res)),
  );
  app.get(
    "/api/v2/mrr/algos",
    asyncHandler(async (req, res) => {
      ...
      if (statusCode === 200 && data?.success && data.data) {
        const items = Array.isArray(data.data)
          ...
          : data.data.algos || [];
        items.forEach((a) => {
          a.nicehashAlgo = normalizeAlgoForNiceHash(a.algo || a.name || a.slug);
        });
      }
      res.set("X-MRR-Client", clientName);
      res.status(statusCode).json(data);
  ...
  app.get(
    "/api/v2/mrr/profiles",
    asyncHandler(async (req, res) => mrrRequest("/profile", req, res)),
  );
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const targetClient = isAggregate(clientQuery)
        ? defaultMrrClient
        : clientQuery;
      const { statusCode, data, clientName } = await mrrApiCall({
  ...
  app.get(
    "/api/v2/mrr/account/pool/:poolIds",
    asyncHandler(async (req, res) => {
      const clientParam = String(
        req.query.client || defaultMrrClient,
      ).toUpperCase();
      const { statusCode, data, clientName } = await mrrApiCall({
        endpoint: `/account/pool/${req.params.poolIds}`,
        clientNameRaw: clientParam,
      });
      ...
      if (statusCode === 200 && data?.success) {
        ...
        items.forEach((item) => {
          const mrrUser = String(item.user || item.username || "")
            .trim()
            .toLowerCase();
          const nhMatch = nhPools.find(
            (nhp) =>
          ...
          if (Array.isArray(item.pools)) {
            item.pools.forEach((p) => {
              const pUser = String(p.user || p.username || "")
                .trim()
                .toLowerCase();
              const pMatch = nhPools.find(
                (nhp) =>
                  String(nhp.username || "")
                    .trim()
                    .toLowerCase() === pUser,
  ...
  app.put(
    "/api/v2/mrr/account/pool",
    asyncHandler(async (req, res) =>
      mrrRequest("/account/pool", req, res, "PUT", req.body),
    ),
  );
  app.put(
    "/api/v2/mrr/account/pool/:poolIds",
    asyncHandler(async (req, res) =>
      mrrRequest(
        `/account/pool/${req.params.poolIds}`,
        req,
        res,
        "PUT",
        req.body,
  ...
  app.delete(
    "/api/v2/mrr/account/pool/:poolIds",
    asyncHandler(async (req, res) =>
      mrrRequest(`/account/pool/${req.params.poolIds}`, req, res, "DELETE"),
    ),
  );
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const rigs = Array.isArray(mrrData?.data?.rigs)
        ? mrrData.data.rigs
        : Array.isArray(mrrData?.data)
          ? mrrData.data
          : [];
      ...
      const uniqueAlgos = [
        ...new Set(
          rigs.map((r) => String(r.algo || r.type || "SHA256").toUpperCase()),
        ),
      ];
      ...
      for (const a of uniqueAlgos) {
        try {
          priceMap.set(
            a,
            await nhApp.hashpower.getOrderPrice({
              algorithm: a,
              market: "USA",
            }),
          );
  ...
  app.get(
    "/api/v2/mrr/rental/history",
    asyncHandler(async (req, res) => {
      const { client: clientQuery, ...forwardQuery } = req.query || {};
      const result = await fetchAggregatedRentals(
        { ...forwardQuery, history: "1" },
        String(clientQuery || defaultMrrClient).toUpperCase(),
      );

      await saveToDatabase(
        "mrr_rental_history.csv",
        result.data?.data?.rentals || [],
      );
  ...
  app.get(
    "/api/v2/mrr/rig/all",
    asyncHandler(async (req, res) => mrrRequest("/rig", req, res)),
  );
  app.get(
    "/api/v2/mrr/whoami",
    asyncHandler(async (req, res) => mrrRequest("/account/whoami", req, res)),
  );
  app.get(
    "/api/v2/mrr/rig",
    asyncHandler(async (req, res) => mrrRequest("/rig", req, res)),
  ...
  app.get(
    "/api/v2/mrr/rig/:rigIds",
    asyncHandler(async (req, res) =>
      mrrRequest(`/rig/${req.params.rigIds}`, req, res),
    ),
  );

  app.get(
    "/api/v2/mrr/rig/:rigIds/pool",
    asyncHandler(async (req, res) => {
      const clientParam = String(
        req.query.client || defaultMrrClient,
      ).toUpperCase();
      const { statusCode, data, clientName } = await mrrApiCall({
        endpoint: `/rig/${req.params.rigIds}/pool`,
        clientNameRaw: clientParam,
      });
    ...
    asyncHandler(async (req, res) => {
      ...
      async function fetchAggressiveRental(clientName) {
        ...
        if (statusCode === 200 && data?.success && rental) {
          ...
          const hasHash =
            initialNorm.niceAverageHashrate !== "0 N/A" &&
            initialNorm.niceAverageHashrate !== "0.00 N/A";
          const hasDuration = initialNorm.duration !== "0";
          if (!hasAlgo || !hasHash || !hasDuration) {
            const listRes = await mrrApiCall({
              endpoint: "/rental",
              clientNameRaw: clientName,
            });
            let list = listRes.data?.success
              ? Array.isArray(listRes.data.data)
                ? listRes.data.data
                : listRes.data.data?.rentals || []
              : [];
            let found = list.find((r) => String(r.id) === String(rentalId));
            if (!found) {
              const histRes = await mrrApiCall({
                endpoint: "/rental",
                query: { history: "1" },
                clientNameRaw: clientName,
              });
              list = histRes.data?.success
                ? Array.isArray(histRes.data.data)
                  ? histRes.data.data
                  : histRes.data.data?.rentals || []
                : [];
          ...
          const poolRes = await mrrApiCall({
            endpoint: `/rental/${rentalId}/pool`,
            clientNameRaw: clientName,
          });
          if (poolRes.statusCode === 200 && poolRes.data?.success) {
            const pData = poolRes.data.data || poolRes.data;
            rental.pools = Array.isArray(pData.pools)
              ? pData.pools
              : Array.isArray(pData)
                ? pData
                : [];
          ...
          if (
            ...
            try {
              const { client: nhClient } = resolveNhClient(clientParam);
              rental.nicehashPrice = await getNiceHashApp(
                nhClient,
              ).hashpower.getOrderPrice({ algorithm: nhAlgo, market: 1 });
            } catch (e) {}
          }
      ...
      if (isAggregate(clientParam)) {
        const clients = Object.keys(mrrConfigs).filter(
          (c) =>
            mrrConfigs[c].apiKey && mrrConfigs[c].apiSecret && !isAggregate(c),
        );
        ...
        const found = candidates.find(
          (c) => c.statusCode === 200 && c.data?.success,
        );
        if (found) {
        ...
        return res
          ...
          .json({
            success: false,
            message: "Rental ID not found in any configured account.",
          });
      }
  ...
  app.get(
    "/api/v2/mrr/rental/:rentalIds/pool",
    asyncHandler(async (req, res) =>
      mrrRequest(`/rental/${req.params.rentalIds}/pool`, req, res),
    ),
  );
  app.get(
    "/api/v2/mrr/rental/:rentalId/hashrate",
    asyncHandler(async (req, res) => {
  ...
  app.get(
    "/api/v2/mrr/rig/:rigIds/info",
    asyncHandler(async (req, res) => {
      const ids = req.params.rigIds
        .split(";")
        .map((id) => id.trim())
        .filter(Boolean);
      if (ids.length === 0)
        return res
          .status(400)
          .json({ success: false, message: "No Rig IDs provided" });
      ...
      const fetchSingleInfo = async (id) => {
        try {
          ...
          if (
            !info.miningAlgorithm ||
            !info.stratumHost ||
            !info.username ||
            !info.password ||
            !info.stratumPort
          ) {
            const rigRes = await mrrApiCall({
              endpoint: `/rig/${id}`,
              clientNameRaw: req.query.client,
            });
          ...
          if (
            ...
            try {
              ...
              info.nicehashPrice = await getNiceHashApp(
                nhClient,
              ).hashpower.getOrderPrice({ algorithm: nhAlgo, market: 1 });
            } catch (e) {}
          }
      ...
      if (ids.length === 1) {
        ...
        res.set(
          "X-MRR-Client",
          String(req.query.client || defaultMrrClient).toUpperCase(),
        );
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      const { scrapeHeroMinersGlobal } =
        await import("./miningOpportunityNotifier.js");
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        ...
        const coinStats = Object.entries(json.result).map(
          ([algorithm, data]) => {
        ...
        res.json({
          success: true,
          miningpooldutch: { coinStats, fetchedAt: new Date().toISOString() },
        });
      } catch (err) {
        res.json({
          success: false,
          error: err.message,
          miningpooldutch: { coinStats: [] },
        });
      }
    }),
  );
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
          ...
          try {
            ...
            const coinStats = Object.entries(json.result).map(
              ([algorithm, data]) => ({
                algorithm,
                coin: algorithm.toUpperCase(),
                btcPerDay: Number.isFinite(
                  parseFloat(data.expected || data.average || 0),
                )
                  ? parseFloat(data.expected || data.average || 0)
                  : 0,
                usdPerDay: 0,
      ...
      res.json({
        ...
        miningpooldutch:
          dutchResult.status === "fulfilled"
            ? dutchResult.value?.miningpooldutch
            : null,
      });
    }),
  );
