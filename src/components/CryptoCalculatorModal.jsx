    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-2">
      <div className="bg-[#1e293b] border border-slate-200 rounded-xl shadow-lg w-full max-w-[280px] overflow-hidden">
        ...
        <div className="px-3 py-2 border-b border-slate-200 flex justify-between items-center bg-slate-800/30">
          <h3 className="font-black text-white tracking-tight">💰 CRYPTO CALC</h3>
        ...
        <div className="p-3 space-y-2">
          ...
          <div className="bg-slate-100/30 p-1 rounded-lg border border-slate-200">
            <label className="font-bold text-slate-400 uppercase tracking-wider block mb-0.5">USD</label>
          ...
          <div className="space-y-1.5 max-h-[100px] overflow-y-auto">
            {loading && !prices ? (
              ...
              <div key={coin.id} className="flex items-center justify-between bg-slate-800/30 px-2 py-1.5 rounded-lg hover:bg-slate-100/50">
                <div className="flex items-baseline gap-1">
                  <span className="font-bold text-white">{coin.symbol}</span>
                  <span className={`font-bold ${coin.change24h >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                ...
                <input
                  ...
                  style={{ fontSize: '1rem' }}
                  value={baseCoin === coin.id ? amounts[coin.id] : (coin.calculated > 0 ? coin.calculated.toFixed(6) : "0")}
        ...
        {/* Refresh */}
        <button onClick={fetchPrices} className="w-full py-1.5 bg-slate-800 font-bold text-slate-400 hover:text-white border-t border-slate-200 uppercase tracking-wider">
          ↻ REFRESH
        </button>
      </div>
    </div>
