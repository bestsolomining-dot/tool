 ...
 */
export default function HashpowerBot({
  algorithm,
  market,
  onCall,
  nhClient = "BT",
  ...
  const addLog = useCallback((message, type = "info") => {
    ...
    setLogs((prev) =>
      [`[${timestamp}] [${type.toUpperCase()}] ${message}`, ...prev].slice(
        0,
        100,
      ),
    );
  ...
  const runIteration = useCallback(async () => {
    ...
    try {
      ...
      if (ordersData?.error || ordersData?.message) {
        ...
        addLog(
          `API Error (Orders): ${typeof errorMsg === "string" ? errorMsg : JSON.stringify(errorMsg)}`,
          "error",
        );
        return;
      }
      ...
      const myOrder = activeOrders.find((o) => {
        ...
        const oMarket =
          typeof o.market === "object"
            ? o.market.id || o.market.name
            : o.market;

        return (
          String(oAlgo || "").toUpperCase() === algorithm.toUpperCase() &&
      ...
      if (!myOrder) {
        addLog(
          `No active order found for ${algorithm} (${market}). Check "My Orders" first.`,
          "warn",
        );
        return;
      }
      ...
      if (bookData?.error || bookData?.message) {
        ...
        addLog(
          `API Error (OrderBook): ${typeof errorMsg === "string" ? errorMsg : JSON.stringify(errorMsg)}`,
          "error",
        );
        return;
      }

      const book = (bookData?.list || bookData?.[0]?.list || []).filter(
        (o) => o.type === "STANDARD",
      );
      ...
      const myLimit =
        parseFloat(currentConfig.limit) > 0
          ? parseFloat(currentConfig.limit)
          : parseFloat(myOrder.limit);
      ...
      const highestCompPrice =
        competitors.length > 0 ? parseFloat(competitors[0].price) : 0;
      ...
      if (mySpeed >= myLimit * 0.95) {
        ...
        if (myPrice > optimalPrice) {
          ...
          addLog(
            `?adjust price?; order ${myOrder.id}, speed ${mySpeed.toFixed(8)}, rigs ${myOrder.rigsCount}, price ${myPrice.toFixed(8)}, step_down -${stepDownDelta.toFixed(4)}`,
            "success",
          );
        } else {
          addLog(
            `[STABLE] Speed OK (${mySpeed.toFixed(5)}). Price is optimal at ${myPrice.toFixed(8)}`,
          );
        }
      } else {
        ...
        if (optimalPrice > myPrice) {
          ...
          addLog(
            `?adjust price?; order ${myOrder.id}, speed ${mySpeed.toFixed(8)}, rigs ${myOrder.rigsCount}, price ${myPrice.toFixed(4)}, step_up ${nextPrice.toFixed(4)}`,
            "warn",
          );
        } else {
          addLog(
            `[WAIT] Already at top of book (${myPrice.toFixed(4)}), waiting for available rigs.`,
          );
        }
      }
      ...
      const limitChanged =
        parseFloat(currentConfig.limit) > 0 &&
        Math.abs(parseFloat(currentConfig.limit) - parseFloat(myOrder.limit)) >
          0.001;
      ...
      if (priceChanged || limitChanged) {
        await onCall(`/api/v2/hashpower/order/${myOrder.id}/update`, {
          ...
          body: {
            ...
            limit: targetLimit,
            displayMarketFactor:
              myOrder.displayMarketFactor ||
              myOrder.algorithm?.displayMarketFactor ||
              book[0]?.displayMarketFactor,
          },
          silent: true,
        });
      }
  ...
  return (
    ...
    >
      ...
      >
        <div className="field">
          ...
          <input
            ...
            onChange={(e) =>
              setConfig((prev) => ({ ...prev, maxPrice: e.target.value }))
            }
            disabled={isRunning}
          />
        </div>
        <div className="field">
          ...
          <input
            ...
            onChange={(e) =>
              setConfig((prev) => ({ ...prev, stepDown: e.target.value }))
            }
            disabled={isRunning}
          />
        </div>
        <div className="field">
          ...
          <input
            ...
            value={config.limit}
            onChange={(e) =>
              setConfig((prev) => ({ ...prev, limit: e.target.value }))
            }
            disabled={isRunning}
          />
        </div>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
        ...
        <button className="btn-pro secondary" onClick={() => setLogs([])}>
          Clear Logs
        </button>
      </div>
