...
export default function MiningCoin({ onCall, nhClient = 'BT' }) {
  ...
  const visibleRows = useMemo(() => {
    ...
    return combinedRows.filter((row) => {
      if (onlyProfitable && !(row.spread > 0 || (row.miningDutchBtcPerDay > 0 && !row.niceHashPrice))) {
        return false;
      }
    ...
    <section className="mining-coin-page" style={{ display: 'grid', gap: '14px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '14px', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button className="btn-pro secondary" onClick={() => void loadData(true)} disabled={loading}>
          ...
          <tbody>
            {loading && !visibleRows.length ? (
              ...
              <tr key={row.nicehashAlgo} style={{ borderBottom: '1px solid rgba(148,163,184,0.08)' }}>
                ...
                <BodyCell>
                  <span style={{
                    color: row.spread > 0 ? '#34d399' : row.spread < 0 ? '#f87171' : '#94a3b8',
                    fontWeight: 700,
                  }}>
                ...
                <BodyCell align="left">
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                    ...
                    {row.heroCoins.length > 8 && <span style={{ color: '#64748b' }}>+{row.heroCoins.length - 8}</span>}
                  </div>
                </BodyCell>
              </tr>
            ))}
...
function SummaryTile({ label, value, tone }) {
  return (
    <div style={{
      ...
      <div style={{ color: tone, fontSize: '18px', fontWeight: 800, marginTop: '6px' }}>{value}</div>
    </div>
  );
}
