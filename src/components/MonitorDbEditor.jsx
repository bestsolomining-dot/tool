...
export default function MonitorDbEditor({ onCall, isOpen, onClose }) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Are you sure you want to remove rental #${id} from the monitor?`)) return;
    try {
      const res = await onCall(`/api/v2/mrr/monitor/snapshot/${id}`, { method: 'DELETE' });
  ...
  const handleResetAlerts = async (id) => {
    try {
      const res = await onCall(`/api/v2/mrr/monitor/snapshot/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ low_hashrate_start: 0, zero_hashrate_start: 0, last_notified: 0 }),
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Monitoring Database Manager" maxWidth="1200px">
      <div className="panel-body" style={{ padding: '1rem', overflowX: 'auto', minHeight: '400px' }}>
        ...
        {data.length > 0 && (
          <table className="select-dropdown-pro" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.1)', opacity: 0.6 }}>
            ...
            <tbody>
              {data.map(row => (
                <tr key={row.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: editingId === row.id ? 'rgba(59, 130, 246, 0.05)' : 'transparent' }}>
                  ...
                  <td style={{ padding: '10px' }}>
                    <span style={{ color: row.low_hashrate_start > 0 ? '#f87171' : 'inherit' }}>{row.low_hashrate_start > 0 ? '⚠️ Active' : 'OK'}</span>
                  </td>
                  <td style={{ padding: '10px' }}>
                    <span style={{ color: row.zero_hashrate_start > 0 ? '#f87171' : 'inherit' }}>{row.zero_hashrate_start > 0 ? '🚫 Zero' : 'OK'}</span>
                  </td>
                  <td style={{ padding: '10px' }}>
