...
export function NiceHashOrderProvider({ children, nhClient, callApi }) {
  ...
  const selectedOrder = useMemo(() => 
    nicehashOrders.find(order => order.id === String(selectedOrderId)) || null,
  ...
  const fetchNiceHashOrders = useCallback(async () => {
    ...
    try {
      ...
      console.log(`[NiceHashOrderContext] Fetched ${list.length} orders for client ${nhClient}`);
      ...
      console.log(`[NiceHashOrderContext] Updated ${combinedList.length} orders`);
...
export const useNiceHashOrders = () => {
  ...
  if (!context) {
    throw new Error('useNiceHashOrders must be used within a NiceHashOrderProvider');
  }
  return context;
};
