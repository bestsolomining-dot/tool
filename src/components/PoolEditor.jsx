...
export default function PoolEditor({
  ...
  useEffect(() => {
    ...
    async function loadDetails() {
      ...
      const result = await callEditorApi(
        poolApi.get(currentPoolId, nhClient),
        "Pool details",
      );
  ...
  const verifyEditorPool = async () => {
    ...
    if (missingFields.length > 0) {
      setEditorError(
        `Missing required verify fields: ${missingFields.join(", ")}`,
      );
      setEditorLoading(false);
      return;
    }
    ...
    try {
      const result = await callEditorApi(
        poolApi.verify(payload, nhClient),
        "Verify pool",
      ); // Pass nhClient
  ...
  const saveEditorPool = async ({ verifyAfterSave = false } = {}) => {
    ...
    if (missingFields.length > 0) {
      setEditorError(
        `Missing required save fields: ${missingFields.join(", ")}`,
      );
      setEditorSaving(false);
      return;
    }
    ...
    try {
      const saveResult = await callEditorApi(
        poolApi.save(savePayload, nhClient),
        "Save pool",
      ); // Pass nhClient
      ...
      const savedId =
        saveResult.data?.id || saveResult.data?.poolId || savePayload.id;
      if (savedId) {
        const detailResult = await callEditorApi(
          poolApi.get(savedId, nhClient),
          "Reload pool details",
        );
  ...
  return (
    <div className={isPopout ? "pool-editor-popout-container" : ""}>
      ...
      >
        ...
        <span style={{ fontSize: "12px" }}>
          {editorDetailsLoading
            ? "Loading details..."
            : editorResponse
              ? ph.isVerifySuccess(editorResponse)
                ? "Verified"
                : "Verification Failed"
      ...
      <div className="pool-editor-grid">
        ...
        <div className="pool-editor-side">
          ...
          <div className="code-block-wrapper">
            ...
            <pre className="code-block-content">
              {editorVerifyBody
                ? JSON.stringify(editorVerifyBody, null, 2)
                : "Invalid JSON or unsupported pool shape."}
            </pre>
          </div>
          ...
          {editorResponse && (
            <div className="code-block-wrapper">
              <div className="code-block-header">
                ...
                <span>
                  {editorLastCall?.label ||
                    (editorResponse.ok ? "Success" : "Fail")}
                </span>
              </div>
              {editorLastCall && (
                <div
                ...
                >
                  <span>
                    {editorLastCall.method} {editorLastCall.path}
                  </span>
                  <span>
                    {editorLastCall.status} ({editorLastCall.durationMs}ms)
                  </span>
                </div>
              )}
              <pre className="code-block-content">
      ...
      {editorError && (
        <pre className="error-message modal-error">{editorError}</pre>
      )}
