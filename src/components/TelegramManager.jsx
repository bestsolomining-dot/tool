...
export function useTelegram(onCall, mrrClient) {
  ...
  const sendManualNotice = useCallback((r) => {
...
export default function TelegramManager({ onCall, mrrClient }) {
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(circle at top left, rgba(14, 165, 233, 0.2), transparent 34%), linear-gradient(135deg, rgba(15, 23, 42, 0.92), rgba(2, 6, 23, 0.86))',
      boxShadow: '0 18px 45px rgba(2, 6, 23, 0.32)'
    }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', flexWrap: 'wrap' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: '13px', fontWeight: 800, marginBottom: '4px', letterSpacing: '0.02em' }}>Telegram Control</div>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: '12px', fontWeight: 800, color: '#e0f2fe' }}>Force heartbeat</div>
          <div style={{ fontSize: '11px', opacity: 0.68 }}>Runs monitor now for <b>{scopeLabel}</b> and sends the Telegram summary.</div>
        </div>
        <button
          ...
          style={{
            minWidth: '118px',
            background: isHeartbeatRunning ? 'rgba(100, 116, 139, 0.16)' : 'linear-gradient(135deg, rgba(14, 165, 233, 0.28), rgba(16, 185, 129, 0.22))',
      ...
      {heartbeatStatus && (
        <div style={{
          ...
          background: heartbeatStatus.includes('failed') ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.09)',
