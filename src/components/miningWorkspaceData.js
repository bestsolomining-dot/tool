...
export function mergeMiningRoutes(miningDutchRows, heroRows, niceHashPrices = {}) {
  ...
  for (const row of heroRows) {
    ...
    if (row.poolHashrate && row.poolHashrate !== 'N/A') current.poolHashrates.push(row.poolHashrate);
  ...
  return Array.from(algos).map((nicehashAlgo) => {
...
export function normalizeMrrMarketRows(payload) {
  ...
  return rentals
    .map((rental) => {
      ...
      const paidValue = getBtcPriceData(rawPrice).value || parsePriceValue(rawPrice?.paid ?? rawPrice?.price ?? rawPrice?.amount ?? rawPrice);
      const durationHours = numberValue(rental?.hours || rental?.length || rental?.duration || rental?.rig?.hours);
      const algo = String(rental?.algo || rental?.algorithm || rental?.rig?.algo || rental?.rig?.algorithm || rental?.rig?.type || 'N/A');
      ...
      const perUnitPerDay = paidValue > 0 && durationHours > 0 && advertised > 0
      ...
      return {
        ...
        durationHours,
        advertised,
        currency: String(rawPrice?.currency || rawPrice?.price_unit || rental?.currency || 'BTC').toUpperCase(),
        mrrClient: rental?.mrrClient || rental?.client || '',
      };
    })
    .filter((row) => row.nicehashAlgo && row.nicehashAlgo !== 'UNKNOWN' && row.pricePerUnitDayBtc > 0);
}

export function buildOpportunityRows(routeRows, niceHashPrices = {}, mrrMarketRows = []) {
  const mrrByAlgo = new Map();
  for (const row of mrrMarketRows) {
    const current = mrrByAlgo.get(row.nicehashAlgo) || { rows: [], price: 0 };
    current.rows.push(row);
    current.price = current.price > 0 ? Math.min(current.price, row.pricePerUnitDayBtc) : row.pricePerUnitDayBtc;
  ...
  return routeRows.map((route) => {
    ...
    const spreadVsNh = nhPrice > 0 ? ((pool - nhPrice) / nhPrice) * 100 : null;
    const spreadVsMrr = mrrMarket > 0 ? ((pool - mrrMarket) / mrrMarket) * 100 : null;
    const winner = [
    ...
    return {
      ...
      opportunityScore: pool - (bestCost === Number.POSITIVE_INFINITY ? 0 : bestCost),
      mrrMarketRows: mrrByAlgo.get(route.nicehashAlgo)?.rows || [],
    };
  }).sort((a, b) => (b.opportunityScore - a.opportunityScore) || (b.poolRevenue - a.poolRevenue));
}
