  ...
  return async function callApi(path, options = {}) {
    ...
    try {
      ...
      if (!isAppError && (res.status === 304 || res.ok)) {
        if (!options.silent) onState?.({ type: 'request-success', payload: { status: res.status, statusText: res.statusText, data } });
      } else if (!options.silent) {
        const errorMsg = typeof data === 'string'
          ? data
          : data?.errors?.[0]?.message || data?.error || data?.message || data?.data?.message || res.statusText;
        onState?.({ type: 'request-error', payload: { status: res.status, errorMsg, data, showModal: !!options.showModal } });
      }
      ...
      if (!options.silent) {
        onState?.({
          type: 'request-failed',
          payload: { error: err.message || String(err), durationMs: Math.round(performance.now() - startedAt) }
        });
      }
      throw err;
    } finally {
